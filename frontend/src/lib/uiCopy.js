...
exports.UI_COPY = {
    ...
    chapterAnalysis: {
        ...
        storyMemorySubtitle: "这里的条目用于回溯定位与检索，可在此手工新增/编辑/合并/完成标记/删除；自动更新（plot_auto_update）也只写入剧情记忆，不会改动世界书。",
    },
    vectorRag: {
    ...
    auth: {
        ...
        linuxdoNotEnabledHint: "LinuxDo 登录未启用：请在后端环境变量中配置 LINUXDO_OIDC_CLIENT_ID / LINUXDO_OIDC_CLIENT_SECRET / LINUXDO_OIDC_REDIRECT_URI。",
    ...
    writing: {
        ...
        memoryInjectionHint: "启用后：生成时会把世界书/记忆等模块的结果注入到提示词；上下文预览也会显示触发条目与最终注入文本。",
    ...
    worldbook: {
        ...
        entriesTitle: "条目列表",
        entriesHint: "关键词条目：命中关键词才会注入。常驻（constant）条目：始终注入、会占用预算，建议少量使用。条目很多时列表会分页渲染，点击“显示更多”继续加载。",
        empty: "暂无条目",
        tagEnabled: "启用",
        tagDisabled: "停用",
        tagBlue: "常驻",
    ...
    featureDefaults: {
        ...
        memoryInjectionHint: "关闭后：章节生成/上下文预览默认不注入世界书/记忆模块；仍可在写作页临时开启（建议用于成本控制/排障）。",
        reset: "恢复默认（开启）",
        resetHint: "仅对当前浏览器生效（localStorage；可随时切换）。",
        toastEnabled: "已开启：写作默认记忆注入",
        toastDisabled: "已关闭：写作默认记忆注入",
        toastReset: "已恢复默认：写作默认记忆注入（开启）",
        autoUpdateHint: "后台自动更新（世界书/图谱/向量/搜索）默认开启：章节定稿/内容变更后会静默调度任务；可在「任务中心」查看状态与重试。",
    },
};
